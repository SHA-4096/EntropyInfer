...
def cache_profilling(fwd_obj, query_states, key_states, block_size, segment_size):
    ...
    last_segment_size = last_query_states.shape[1]
...
def eattention(fwd_obj, 
    ...
    if input_len == 1:
        ...
        for i in range(0, input_len, segment_size):
            ...
            # )
            q_len = q_segment.size(1)
            k_len = k_segment.size(1)
            # Match flash_attention(causal=True) for non-square attention by
            # using a bottom-right aligned causal mask.
            attention_mask = torch.triu(
                torch.ones((q_len, k_len), device=q_segment.device, dtype=torch.bool),
            ...
            if NPU_ATTN_INFR:
                attn_output, _ = torch_npu.npu_fused_infer_attention_score(
                    ...
                    k_segment,
                    v_segment,
                    num_heads=fwd_obj.num_heads,
                    num_key_value_heads=fwd_obj.num_key_value_heads,
                    input_layout="BSND",
                    atten_mask=attention_mask,
                    scale=fwd_obj.scaling,
                )
            else:
                raise RuntimeError(
                    "Sparse prefill attention requires either NPU fused attention or CUDA flash-attn."
                )
            # make sure attn_output shape is BSND
            assert attn_output.shape[1] == q_segment.size(1), f"Unexpected attn_output shape: {attn_output.shape}"
...
def compress_kv(self, origin_key_states, query_states, origin_value_states):
    # expect shape: (bsz, seqlen, num_heads_k, head_dim)
    # TODO: perhaps we can directly accept (bsz,  seqlen, num_heads_k, head_dim) in the future without transpose
    # first transpose to (bsz, num_heads_k, seqlen, head_dim) for compatibility
    origin_key_states = origin_key_states.transpose(1, 2)
    origin_value_states = origin_value_states.transpose(1, 2)
    ...
    if q_len < decode_evict_budgets:
        ...
        # support gqa
        if use_gqa:
            return origin_key_states.transpose(1, 2), origin_value_states.transpose(1, 2)
        else:
